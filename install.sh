#!/usr/bin/env bash
#
# Install Mason from GitHub Releases.
#
#   curl -fsSL https://raw.githubusercontent.com/provasign/mason/main/install.sh | bash
#
...
info "Registering mason with detected AI coding tools (global)…"
"${INSTALL_DIR}/mason" init --global 2>/dev/null \
  && ok "mason registered globally with detected AI tools" \
  || info "mason global init skipped (run: mason init --global)"
...
if [ -n "${PROJECT:-}" ]; then
  ...
  ( cd "$PROJECT"
    "${INSTALL_DIR}/mason" init >/dev/null 2>&1 \
      && ok "mason: project initialized" \
      || err "mason init failed — run manually: mason init"
    "${INSTALL_DIR}/mason" index >/dev/null 2>&1 \
      && ok "mason: index built" \
      || err "mason index failed — run manually: mason index"
  )
fi

printf '\n%s %s installed. Open a new terminal or run:\n  export PATH="%s:$PATH"\n\nAI tool note:\n  Restart or reload your coding agent / IDE so it respawns MCP servers from the updated config.\n  For Claude Code, approve the .mcp.json servers if prompted, then verify with: claude mcp list\n\nNext: cd /your/project && mason init && mason index\n' \
